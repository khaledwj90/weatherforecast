...
import {FieldLabelStyle, GetMainFieldType, SharedStyles, StyleFieldBasedOnProps} from "../_sharedStyle";
...
const InputField = (props: InputFieldProps) => {
    ...
    const handleText = React.useCallback((newValue) => {
        const hasFormat = typeof props.fieldFormatter === 'function';
        if (props.form) {
            props.form.handleChange(props.field.name)(hasFormat ? props.fieldFormatter(newValue) : newValue);
        } else {
    ...
    const textInputStyle = () => {
        const isInvalid = props.form && props.form.errors && props.form.errors[props.field.name] && props.form.touched[props.field.name] === true;
        const isPhoneField = props.keyboardType === 'phone-pad';
        return StyleFieldBasedOnProps(isFocused, props.styleType, isInvalid, isPhoneField, props.disabled === true);
    }
    ...
    const fieldValue = () => {
        if (typeof props.fieldFormatter === 'function') {
            return (
                props.field ? props.fieldFormatter(props.field.value) : props.fieldFormatter(props.value)
            )
        } else {
        ...
        <View style={[props.style]}>
            ...
            <View style={{justifyContent: 'center'}}>
                    ...
                    returnKeyType={props.returnKeyType}/>
                {
                    ...
                    <Text size={4}
                          style={[Platform.OS === 'ios' ? style.mobileStartCodeText : style.mobileStartCodeTextAndroid, getLabelStyleFromType()]}>
            ...
            {
                props.form && props.form.errors && props.form.errors[props.field.name] && props.form.touched[props.field.name] === true ?
